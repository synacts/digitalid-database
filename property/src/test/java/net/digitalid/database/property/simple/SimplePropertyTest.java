...
abstract class CustomString {
    ...
    @Pure
    public abstract @Nonnull @MaxSize(50) String getValue();
...
@GenerateSubclass
@GenerateConverter
abstract class ClassWithSimpleProperty {
    ...
    protected final @Nonnull ObjectModule<ClassWithSimpleProperty> MODULE = ObjectModuleBuilder.<ClassWithSimpleProperty>withParentModule(null).withName("ClassWithSimpleProperty").withConverter(ClassWithSimplePropertyConverter.INSTANCE).build();
    
    @Pure
//    @GenerateModule
    protected void generateModule() {}
    
    protected final @Nonnull SimplePropertyEntryConverter<ClassWithSimpleProperty, CustomString> converter = SimplePropertyEntryConverterBuilder.<ClassWithSimpleProperty, CustomString>withName("ClassWithSimpleProperty_name").withObjectConverter(ClassWithSimplePropertyConverter.INSTANCE).withValueConverter(CustomStringConverter.INSTANCE).build();
    
    protected final @Nonnull PropertyTable<ClassWithSimpleProperty, CustomString> table = PropertyTableBuilder.<ClassWithSimpleProperty, CustomString>withName("name").withParentModule(MODULE).withConverter(converter).withValueValidator(Predicate.ALWAYS_TRUE).build();
...
public class SimplePropertyTest extends CustomTest {
    ...
    public void testProperty() throws DatabaseException {
        ...
        object.name().set(CustomStringBuilder.withValue("test").build());
        object.name().reset();
        assertEquals("test", object.name().get().getValue());
    }
